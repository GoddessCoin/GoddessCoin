...
unsigned int nRetarget = 60;
unsigned int nStakeMinAge = 60 * 60 * 24 * 1; // 1 day

unsigned int nStakeMaxAge = -1; //30 day [-1 is unlimited]
...
const string strMessageMagic = "GoddessCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // GoddessCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // GoddessCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if (pindexBest->nHeight >= 1 && pindexBest->nHeight <= 10080)
      {
        nSubsidy = 153 * COIN;
        if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfWorkReward() : create=%s nSubsidy=%"PRId64"\n", FormatMoney(nSubsidy).c_str(), nSubsidy);
...
//int64_t GetProofOfStakeReward(int64_t nCoinAge, unsigned int nBits, int64_t nFees, int nHeight)
{
    ...
    nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE;
			...
			if(pindexBest->nHeight < YEARLY_BLOCKCOUNT)
				nRewardCoinYear = 9 * MAX_MINT_PROOF_OF_STAKE; //first year 20%
			else if(pindexBest->nHeight < (2 * YEARLY_BLOCKCOUNT))
				nRewardCoinYear = 7 * MAX_MINT_PROOF_OF_STAKE; // year 2 15%
			else if(pindexBest->nHeight < (3 * YEARLY_BLOCKCOUNT))
				nRewardCoinYear = 5 * MAX_MINT_PROOF_OF_STAKE; //year 3 10%
			else if(pindexBest->nHeight < (4 * YEARLY_BLOCKCOUNT))
				nRewardCoinYear = 3 * MAX_MINT_PROOF_OF_STAKE; //year 4 8%
			
    int64_t nSubsidy = nCoinAge * nRewardCoinYear / 365 / COIN;
    //int64_t nSubsidy = nCoinAge * nRewardCoinYear * 33 / (365 * 33 + 24);
...
// GoddessCoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes GoddessCoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // GoddessCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // GoddessCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // GoddessCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
        if (!vtx[1].GetCoinAge(txdb, nCoinAge))
            return error("ConnectBlock() : %s unable to get coin age for coinstake", vtx[1].GetHash().ToString().substr(0,10).c_str());

        int64_t nCalculatedStakeReward = GetProofOfStakeReward(nCoinAge, nFees); //---------#####------
        //int64_t nCalculatedStakeReward = GetProofOfStakeReward(nCoinAge, nFees,pindexBlock->nHeight);
        if (nStakeReward > nCalculatedStakeReward)
            return DoS(100, error("ConnectBlock() : coinstake pays too much(actual=%"PRId64" vs calculated=%"PRId64")", nStakeReward, nCalculatedStakeReward));
    }

    // GoddessCoin: track money supply and mint amount info
...
// GoddessCoin: total coin age spent in transaction, in the unit of coin-days.
...
// GoddessCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // GoddessCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // GoddessCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // GoddessCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // GoddessCoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // GoddessCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // GoddessCoin: check proof-of-stake
    ...
    // GoddessCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // GoddessCoin: getblocks may not obtain the ancestor block rejected
    ...
    // GoddessCoin: if responsible for sync-checkpoint send it
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    {
        if (wallet.GoddessCoinStake(wallet, nBits, nSearchTime-nLastCoinStakeSearchTime, nFees, txCoinStake, key))
        {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "GoddessCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "GoddessCoin is X13 POW and POS";
        CTransaction txNew;
        txNew.nTime = 1405065503;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1405065503;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 8768983;
        if(fTestNet)
        {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x87619fc8a23f108d40ae8804a672f7220a4454bd945ee52edbbcb64e051beb29"));
        block.print();
        ...
        // GoddessCoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0x70, 0x42, 0x13, 0x05 };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // GoddessCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // GoddessCoin: tell downloading node about the latest block if it's
